...
def main():
    ...
    def select_action(state, epsilon):
        if np.random.rand() <= epsilon:
            ...
            q_values = agent(state_tensor)
            valid_actions = env.get_valid_actions()
            mask = torch.ones(output_dim).to(device) * float('-inf')
            mask[valid_actions] = 0
            masked_q_values = q_values + mask
            action = torch.argmax(masked_q_values).item()
        return action
    ...
    def update_priorities(self, indices, priorities):
        for idx, prio in zip(indices, priorities):
            self.priorities[idx] = prio  # Ensure prio is a scalar when passed here
    ...
    env = ConnectFourEnv()
    agent = TransformerAgent(input_dim, embed_dim, n_heads, ff_dim, n_layers, output_dim, dropout).to(device)
    print(f'Agent: {agent}  Device: {device}')
    ...
    for epoch in range(num_epochs):
        ...
        loss = torch.tensor(0.0).to(device)  # Initialize loss to zero or another default value
        ...
        while not done:
            ...
            if len(replay_buffer) >= batch_size:
                samples, weights, indices = replay_buffer.sample(batch_size)
                state_batch, action_batch, reward_batch, next_state_batch, done_batch = zip(*samples)

                state_batch = torch.stack([torch.tensor(state, dtype=torch.float32, device=device) for state in state_batch])
                action_batch = torch.LongTensor(action_batch).to(device)
                reward_batch = torch.FloatTensor(reward_batch).to(device)
                next_state_batch = torch.stack([torch.tensor(state, dtype=torch.float32, device=device) for state in next_state_batch])
                ...
                # Ensure target_q_values is squeezed to remove unnecessary dimensions
                target_q_values = (reward_batch + gamma * max_next_q_values * (1 - done_batch))
                # Also ensure q_value is properly squeezed
                ...
                # Check dimensions and squeeze if it has an extra dimension:
                if target_q_values.dim() > 1 and target_q_values.shape[1] == 1:
                    target_q_values = target_q_values.squeeze(1)
                # Using the criterion for calculating loss:
                loss = (criterion(q_value, target_q_values) * weights).mean()
                ...
                # Update priorities in the replay buffer
                new_priorities = torch.abs(target_q_values - q_value).detach().cpu().numpy() + 1e-6  # add a small constant to ensure no zero priority
